...
with open(jsonfile, 'r') as file:
    ...
    all_best_params = json.loads(json_string)
...
for d in range(ND):
    ...
    k = np.linspace(0.2, 1.2, 11)
    max_recall = 0
    for k in np.linspace(0.5, 1.5, 41):
        print('.', end='')
        recall, std = evaluate_model(models[d], X=x_data, y=y_data[d], num_folds=5, th=k * ths[d])
    ...
    kk[d] = max_k
    print()
    print("Best k=%.2f recall%d=%.4f std=%.4f th[]=%.4f" % (max_k, d, max_recall, max_std, ths[d]))
avg_recall /= ND
var /= (ND - 1)
...
for d in range(ND):
    ...
    y_preds[d] = get_pred(test_probs, kk[d] * ths[d])
...
name = '%s-%04d-%d.csv' % (host, N_EVALS, seed)
print(name)
