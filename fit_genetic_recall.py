...
N_BOTS = 20  # размер популяции
N_GEN = 5 # число поколений
K_FOLDS = 5
seed = 5642
jsonfile = 'configs-%d-%d-%d.json' % (N_BOTS, N_GEN, seed)
dis = list(range(5))
ND = len(dis)
...
class GA:
    def __init__(self, n, d, split=0.3, reverse=True):
        """
    ...
    def sort_bots(self):
        self.popul = sorted(self.popul, key=lambda x: x.recall, reverse=self.reverse)
...
def cross_validation(config, X, y, num_folds):
    ...
    for train_index, val_index in kf.split(X):
        ...
        y_pred = get_pred(val_probs, ths[config['d']])
...
def objective(params):
    ...
    mean_cv = np.mean(cv)
    std_cv = np.std(cv)
    return {'recall': mean_cv, 'std': std_cv, 'params': params}
...
for d in dis:
    ...
    ga = GA(N_BOTS, d)
    bot = ga.popul[0]
    print("d=%d generation %d loss=%.4f time=%.1f" % (d, 0, bot.recall, time() - t0))
    for i in range(N_GEN):
        t0 = time()
        ga.evolution()
        bot = ga.popul[0]
        print("d=%d generation %d loss=%.4f time=%.1f" % (d, i+1, bot.recall, time() - t0))
...
for seed in range(1, 17, 2):
    ...
    avg_recall /= ND
    var /= ND
    print("Estimate Recall=%.4f std=%.4f seed=%d" % (avg_recall, sqrt(var), seed))
